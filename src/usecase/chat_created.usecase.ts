...
import { ChatCreatedEvent, MessagingNotification } from "../ports/pubsub.api.js";
...
export class ChatCreated {
    ...
    async run(event: ChatCreatedEvent) {
        const transaction = await this._blockchainRepository.getTransaction(event.smartContractGenesisAddress);
        if (transaction === null) {
        ...
        if (await (new HasNotificationAlreadyBeenSent).guard(event.smartContractGenesisAddress)) {
            console.log(`Notification rejected : already sent for chat ${event.smartContractGenesisAddress} creation`)
        ...
        const notification: MessagingNotification = {
            ...
            txAddress: event.smartContractGenesisAddress,
